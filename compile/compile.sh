...
docker run -v "$projectDir":/sources ethereum/solc:"$2" --allow-paths="$contractDir" -o "$compiledDir" --optimize --optimize-runs 200 --evm-version cancun --overwrite --bin --abi "$contractDir/$1.sol"
