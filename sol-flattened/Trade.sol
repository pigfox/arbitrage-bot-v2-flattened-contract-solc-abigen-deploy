...
contract Trade {
    ...
    uint private transferThreshold;
    ...
    function setTransferThreshold(uint _transferThreshold) external onlyOwner{
        transferThreshold = _transferThreshold;
    }
    ...
    function Swap(
        ...
        // Transfer SUNToken from user to this contract
        bool success = token.transferFrom(msg.sender, address(this), _amount);
        require(success, "Token transfer failed");

        // Deposit tokens to the source DEX
        token.approve(_sourceDexAddress, _amount);
        sourceDex.deposit(_amount);

        // Withdraw tokens from the source DEX
        sourceDex.withdraw(_amount);

        // Deposit tokens to the destination DEX
